...
const Swiper = function(swiper) {
    ...
    this.touchStartHandle = e => {    
        this.touchStartTime = 0;
        console.log(this.swiperItemWidth);
    ...
    this.touchEndHandle = e => {
        ...
        if ((Math.abs(dx) > this.swiperItemWidth / 3) || (timeGap < 300 && Math.abs(dx) > 30)) {
            if (dx > 0) {
                this.swiperMovePrev();
            } else {
                this.swiperMoveNext();
    ...
    this.setTransform = function (dx) {
        ...
        for (let key in this.indexObj) {
            ...
            this.swiperItems[this.indexObj[key]].style.transform = `translateX(${10 * slideParam + dx}rem)`
        }
    }

    this.setTransition = function (left,current,right) {
        Array.prototype.map.call(this.swiperItems,item => item.style.transition = 'none')
        if (left) {
